...
void IMU_AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) 
{
 ...
 #define MagnetcDeclination 	4.43f //±ÊÕßËùÔÚµØ´ÅÆ«½Ç
 #ifndef PI
#define PI                 			3.14159265358979f
#endif
int IMU_Calculate(int16_t inX,int16_t inY,int16_t inZ)
{
	float headingRadians = atan2((double)(inZ),(double)(inX));
	if(headingRadians < 0)  
	{
		headingRadians += 2*PI;
	}
	int headingDegrees = headingRadians * 180/M_PI; 
	headingDegrees += MagnetcDeclination; //´ÅÆ«½Ç 
...
void IMU_GetQuater(void)
{
	...
	MPU9250_Get3AxisMagnetRawData_ZL(&mag);
	//printf("X:%d Y:%d Z:%d\r\n",mag.x,mag.y,mag.z);
	//IMU_Calculate(mag.x,mag.y,mag.z);
       MotionVal[0]=gyro.x/32.8f;
       MotionVal[1]=gyro.y/32.8f;
       MotionVal[2]=gyro.z/32.8f;
       MotionVal[3]=accel.x;
       MotionVal[4]=accel.y;
       MotionVal[5]=accel.z;
       MotionVal[6]=mag.x;
