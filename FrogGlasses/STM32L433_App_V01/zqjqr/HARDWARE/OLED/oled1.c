...
{		  
	...
	if(cmd == OLED_CMD) HAL_GPIO_WritePin(GPIOA,GPIO_PIN_6,GPIO_PIN_RESET);//OLED_RS_OFF;
	else HAL_GPIO_WritePin(GPIOB,GPIO_PIN_6,GPIO_PIN_SET);//OLED_RS_ON;

	for(i=0;i<8;i++)
	{			  
		OLED_SCLK_OFF;
		if(dat&0x80) HAL_GPIO_WritePin(GPIOA,GPIO_PIN_7,GPIO_PIN_SET);//OLED_SDIN_ON;//OLED_SDIN=1;
		else HAL_GPIO_WritePin(GPIOA,GPIO_PIN_7,GPIO_PIN_RESET);//OLED_SDIN_OFF;//OLED_SDIN=0;
		OLED_SCLK_ON;
		dat<<=1;   
	}				 
